...
func Mkusr(parametros []string) {
	...
	for _, parametro := range parametros[1:] {
		...
		if len(tmp) != 2 {
			fmt.Println("MKUSR Error: Valor desconocido del parametro ", tmp[0])
			paramC = false
			break
		}
		...
		if strings.ToLower(tmp[0]) == "user" {
			...
			if len(user) > 10 {
				fmt.Println("MKUSR ERROR: user debe tener maximo 10 caracteres")
				paramC = false
				return
			}
			...
			grp = tmp[1]
			if len(grp) > 10 {
				fmt.Println("MKUSR ERROR: grp debe tener maximo 10 caracteres")
				paramC = false
				return
			}
			...
			if usuarioActual.Status {
				if usuarioActual.Nombre == "root" {
					...
					if ejecutar {
						...
						grupo := true // si el grupo existe. true -> no existe
						for _, registro := range lineaID[:len(lineaID)-1] {
							datos := strings.Split(registro, ",")
							if len(datos) == 3 {
								if datos[2] == grp {
									//verificar que no este borrado logicamente
									if datos[0] != "0" {
										grupo = false
										break
									}
						...
						if grupo {
							fmt.Println("MKUSR ERROR: El grupo no existe")
							return
						}

						//Verificar si el usuario ya existe indistintamente del grupo
						for _, registro := range lineaID[:len(lineaID)-1] {
							datos := strings.Split(registro, ",")
							if len(datos) == 5 {
								if datos[3] == user {
									fmt.Println("MKUSR ERROR: El usuario ya existe")
									return
								}
							}
						}
						...
						if id != -1 {
							...
							if posicionNulo != -1 {
								...
								if libre > 0 {
									...
									if guardoInfo {
										fmt.Println("MKUSR ERROR: Espacio insuficiente para nuevo registro")
									}
								}
							}
						}
					...
					fmt.Println("MKUSR ERROR: Usuario con permisos insuficientes")
				}
			} else {
				fmt.Println("MKUSR ERROR: Sesion no iniciada")
			}
		} else {
			fmt.Println("MKUSR ERROR: Falta alguno de los siguientes parametros -> user. pass o grp")
		}
	}
}
